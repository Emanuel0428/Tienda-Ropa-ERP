...
import { getWeekSchedule } from '../services/attendanceService';
import { formatDeadline, getWeekNumber, getMonday } from '../hooks/useAttendanceUtils';
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (userData.rol !== 'administradora' && userData.rol !== 'asesora') return;
        ...
        const weekSchedule: DaySchedule[] = Array.from({ length: 7 }, (_, i) => {
          ...
          return {
            day: DAY_NAMES[dayOfWeek].slice(0, 3).toUpperCase(),
    ...
    return (
      <div className="p-6">
        <Card className="max-w-2xl mx-auto p-8 text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">Horario no disponible</h2>
          <p className="text-gray-600">Esta función está disponible solo para Administradoras y Asesoras.</p>
        </Card>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-6xl mx-auto pt-20">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">{weekInfo.startDate} - {weekInfo.endDate}</p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {schedule.map((day, index) => {
          ...
          return (
            ...
            >
              <div className="text-center">
                <div className={`text-sm font-bold mb-2 pb-2 border-b-2 ${isToday ? 'text-blue-600 border-blue-500' : 'text-gray-600 border-gray-200'}`}>
                  {day.day}
                </div>
                <div className="text-xs text-gray-500 mb-3">{day.date}</div>
                {isToday && (
                  <div className="mb-3">
                    <span className="inline-block bg-blue-500 text-white text-xs font-semibold px-2 py-1 rounded-full">HOY</span>
                  </div>
                )}
                {day.is_day_off ? (
                  <div className="py-6">
                  ...
                  <div className="py-4">
                    ...
                    <p className="text-2xl font-bold text-gray-800">{formatDeadline(day.check_in_deadline)}</p>
                    <p className="text-xs text-gray-500 mt-1">Hora de entrada</p>
                    {day.notes && (
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <p className="text-xs text-gray-600 italic">📝 {day.notes}</p>
                      </div>
                    )}
                  </div>
                )}
      ...
      <Card className="mt-6 bg-gray-50">
        <div className="flex flex-wrap items-center justify-center gap-6 text-sm">
          <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-blue-500"></div><span className="text-gray-600">Día actual</span></div>
          <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-gray-200"></div><span className="text-gray-600">Día libre</span></div>
          <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-gray-500" /><span className="text-gray-600">Hora de entrada programada</span></div>
        </div>
      </Card>
